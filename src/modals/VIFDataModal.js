        ...
        if (port.fileJson && port.deviceJson === undefined) {
            if (port.fileJson.VIF.VIF_App)
                if (port.fileJson.VIF.VIF_App.Version["_text"].split('.').join("") > Constants.VIF_SUPPORTED_VERSION.split('.').join("")) {
                    basemodal.showPopUp("The current version of software only supports VIF files with version upto " + [Constants.VIF_SUPPORTED_VERSION] + ".There might be issues with decoding the VIF data. ", null, 'VIF Version Support', null, null, null, null, null)
                }
        }
