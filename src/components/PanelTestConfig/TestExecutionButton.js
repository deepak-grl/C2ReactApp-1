        ...
        render() {
            ...
            return (<FlexView column className="testexecutionbutton-container">
                <OverlayTrigger placement="auto" trigger="hover" delay={{ show: 0, hide: 1 }} overlay={<Tooltip> {buttonToolTip} </Tooltip>}>
                    <Button id="tcStartAndStopExecutionBtn" onClick={this.runHandler} disabled={!this.isDisabled || mainstore.configControllerCaptureInProgress} className={buttonClass}>{buttonText}</Button>
                </OverlayTrigger>
            </FlexView >);
        }
    })
